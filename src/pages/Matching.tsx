// Updated Matching component with query param handling and top 3 results display
...
const Matching = () => {
  ...
  // If URL contains userId param, select that user automatically
  useEffect(() => {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <Card className="p-4 bg-muted border rounded-lg">
            <div className="flex flex-wrap gap-3 items-center">
              <div className="flex items-center gap-2"><Checkbox id="filterForeigners" checked={filterForeigners} onCheckedChange={(c) => setFilterForeigners(!!c)} /><label htmlFor="filterForeigners" className="text-sm">외국인만</label></div>
              <div className="flex items-center gap-2"><Checkbox id="filterWeekend" checked={filterWeekend} onCheckedChange={(c) => setFilterWeekend(!!c)} /><label htmlFor="filterWeekend" className="text-sm">주말가능</label></div>
              <div className="w-full flex flex-wrap gap-2 items-center">
                {SUPPORT_TYPES.map((type) => (
                  <div key={type} className="flex items-center gap-1.5">
                    <Checkbox
