...
const WorkBoard = () => {
  ...
  const monthAgenda = eventsForCalendarMonth(calendarDisplayEvents, calendarMonth.getFullYear(), calendarMonth.getMonth());
    ...
    <div className="space-y-6">
          ...
          <CardContent className="space-y-4 px-4 pt-5 sm:px-6">
            ...
            <div className="max-h-80 divide-y overflow-y-auto">
              {!visibleTodos.length ? <p className="py-10 text-center text-sm text-muted-foreground">표시할 할 일이 없습니다.</p> : visibleTodos.map((todo) => (
                <div key={todo.id} className={cn("flex flex-wrap items-center gap-2 rounded-md px-2 py-3 sm:flex-nowrap", todo.completed && "my-1 border border-dashed bg-muted/50 opacity-75")}>
                  <Checkbox checked={todo.completed} onCheckedChange={(checked) => todo.id && void todosStore.update(todo.id, { completed: checked === true })} />
                  <span className={cn("min-w-0 basis-[calc(100%-2rem)] break-words text-sm font-medium sm:basis-auto sm:flex-1", todo.completed && "text-xs font-normal italic text-muted-foreground line-through")}>{todo.title}</span>
      ...
      <Card onDragEnd={() => setDraggedWidget(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => void dropWidget("calendar")} style={{ order: widgetOrder.indexOf("calendar") }} className={cn("overflow-hidden cursor-move xl:col-span-2", draggedWidget === "calendar" && "opacity-60")}>
        ...
        <CardContent className="p-0 sm:p-4">
          ...
          <div data-no-swipe className="grid grid-cols-7 gap-px bg-border">{calendarDays.map((day, dayIndex) => {
            ...
            return <div key={day.date} role="button" tabIndex={0} className={cn("min-h-20 sm:min-h-28 min-w-0 bg-background p-1 transition hover:bg-muted/30", !day.inMonth && "bg-muted/20 text-muted-foreground", calendarDropDate === day.date && "bg-primary/10 ring-2 ring-inset ring-primary")} onClick={() => setSelectedCalendarDate(day.date)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelectedCalendarDate(day.date); }} onDragOver={(event) => { if (!calendarDrag) return; event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDragEnter={(event) => { if (!calendarDrag) return; event.preventDefault(); setCalendarDropDate(day.date); }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setCalendarDropDate(null); }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); void dropCalendarEvent(day.date); }}>
              ...
              <div className="space-y-0.5">{Array.from({ length: laneCount }, (_, lane) => {
                ...
                return <div key={event.id} role="button" tabIndex={0} draggable={canDrag} title={`${event.title}${event.startTime ? ` · ${event.startTime}~${event.endTime}` : ""}${event.note ? ` · ${event.note}` : ""}`} className={cn("relative flex h-5 w-[calc(100%+4px)] items-center overflow-hidden px-1 text-left text-[10px] font-semibold leading-5 shadow-sm", calendarColorClass[event.color], canDrag ? "cursor-grab active:cursor-grabbing" : "cursor-pointer", starts && "ml-0 rounded-l-md", !starts && "-ml-1", ends && "w-full rounded-r-md")} onDragStart={(dragEvent) => canDrag && startCalendarDrag(dragEvent, event.id, "move", day.date)} onDragEnd={() => { setCalendarDrag(null); setCalendarDropDate(null); }} onClick={(clickEvent) => { clickEvent.stopPropagation(); setSelectedCalendarEvent(event); }} onKeyDown={(keyEvent) => { if (keyEvent.key === "Enter" || keyEvent.key === " ") setSelectedCalendarEvent(event); }}>
                  {canDrag && starts && <span draggable className="mr-1 hidden sm:flex h-full w-2 shrink-0 cursor-ew-resize items-center justify-center border-r border-white/60" title="시작일 조절" onDragStart={(dragEvent) => startCalendarDrag(dragEvent, event.id, "start", day.date)}>‹</span>}
                  <span className="min-w-0 flex-1 truncate">{starts || dayIndex % 7 === 0 || day.day === 1 ? `${event.startTime ? `${event.startTime} ` : ""}${event.title}` : ""}</span>
                  {canDrag && ends && <span draggable className="ml-1 hidden sm:flex h-full w-2 shrink-0 cursor-ew-resize items-center justify-center border-l border-white/60" title="종료일 조절" onDragStart={(dragEvent) => startCalendarDrag(dragEvent, event.id, "end", day.date)}>›</span>}
                </div>;
              })}</div>
            </div>;
          })}</div>
          <div className="space-y-2 border-t p-3 sm:hidden" data-no-swipe>
            <p className="text-sm font-semibold">이달의 일정 · 눌러서 상세 보기</p>
            {!monthAgenda.length && <p className="py-3 text-sm text-muted-foreground">등록된 일정이 없습니다. 달력의 날짜를 눌러 추가하세요.</p>}
            {monthAgenda.map((event) => <button key={event.id} type="button" className="flex w-full min-w-0 items-start gap-3 rounded-lg border bg-background p-3 text-left" onClick={() => setSelectedCalendarEvent(event)}>
              <span className={cn("mt-1 h-10 w-1 shrink-0 rounded-full", calendarColorClass[event.color].split(" ")[0])} />
              <span className="min-w-0 flex-1"><span className="block break-words text-sm font-semibold">{event.title}</span><span className="mt-1 block text-xs text-muted-foreground">{event.startDate}{event.endDate !== event.startDate ? ` ~ ${event.endDate}` : ""}</span><span className="block text-xs font-medium text-primary">{event.startTime ? `${event.startTime} ~ ${event.endTime}` : "종일"}{event.source === "annual" ? " · 연간 일정" : ""}</span></span>
            </button>)}
          </div>
        </CardContent>
      </Card>
      ...
      <Card onDragEnd={() => setDraggedWidget(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => void dropWidget("matching")} style={{ order: widgetOrder.indexOf("matching") }} className={cn("cursor-move xl:col-span-2", draggedWidget === "matching" && "opacity-60")}>
        ...
        <CardContent className="px-4 pt-5 sm:px-6">
          {!matchingItems.length ? <p className="py-10 text-center text-sm text-muted-foreground">현재 매칭이 필요한 등록 대상이 없습니다.</p> : <div className="grid gap-3 md:grid-cols-2">{matchingItems.map((item) => {
            ...
            return (
            <div key={item.id} className="relative flex flex-col gap-3 rounded-xl border p-3 pr-12 sm:flex-row sm:items-start sm:p-4">
              <div className="flex shrink-0 flex-row gap-1 sm:flex-col"><Badge variant="outline">{item.targetType}</Badge><Badge variant={item.matchMode === "1:다" ? "default" : "secondary"}>{item.matchMode || "1:1"}</Badge></div>
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 break-words whitespace-pre-wrap text-sm text-muted-foreground">{item.condition}</p>
              ...
              <Button variant="ghost" size="icon" className="absolute right-1 top-1 sm:static sm:shrink-0" aria-label={`${item.targetName} 제거`} onClick={() => item.id && void matchingStore.remove(item.id)}><X className="h-4 w-4" /></Button>
            </div>
          );})}</div>}
        </CardContent>
      </Card>
      ...
      <Card onDragEnd={() => setDraggedWidget(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => void dropWidget("annualSchedules")} style={{ order: widgetOrder.indexOf("annualSchedules") }} className={cn("cursor-move xl:col-span-2", draggedWidget === "annualSchedules" && "opacity-60")}>
        ...
        <CardContent className="px-4 pt-5 sm:px-6">
          <div className="space-y-3 sm:hidden">
            ...
            {schedules.map((schedule) => <div key={schedule.id} className="space-y-2 rounded-lg border p-3">
              <div className="flex flex-wrap items-center gap-2"><h3 className="min-w-0 flex-1 break-words font-semibold">{schedule.projectName}</h3><Badge variant="outline" className={statusClass[schedule.status]}>{schedule.status}</Badge></div>
              <p className="text-sm">준비 시작: <span className="font-semibold text-primary">{schedule.preparationStartDate || "미등록"}</span></p>
              <p className="whitespace-pre-wrap text-sm">{formatScheduleMilestones(schedule)}</p>
              {schedule.note && <p className="whitespace-pre-wrap text-xs text-muted-foreground">{schedule.note}</p>}
              <div className="flex items-center justify-between gap-2"><span className="text-xs text-muted-foreground">담당: {schedule.manager}</span><div className="flex gap-1"><Button variant="outline" size="sm" onClick={() => openScheduleDialog(schedule)}>수정</Button><Button variant="ghost" size="sm" className="text-destructive" onClick={() => schedule.id && confirm("이 일정을 삭제할까요?") && void scheduleStore.remove(schedule.id)}>삭제</Button></div></div>
            </div>)}
          </div>
          <div className="hidden overflow-x-auto sm:block"><table className="w-full min-w-[960px] text-sm"><thead><tr className="border-b bg-muted/40"><th className="px-3 py-3 text-left">연간사업명</th><th className="px-3 py-3 text-left">상태</th><th className="px-3 py-3 text-left">업무준비 시작일</th><th className="px-3 py-3 text-left">시행날짜</th><th className="px-3 py-3 text-left">비고</th><th className="px-3 py-3 text-left">담당</th><th className="px-3 py-3 text-right">관리</th></tr></thead><tbody>
