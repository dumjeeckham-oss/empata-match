...
>(({ className, ...props }, ref) => (
  <DialogContent
    ...
    data-no-swipe
    className={cn(
      "left-0 top-0 flex h-[100dvh] max-h-[100dvh] w-screen min-w-0 max-w-none translate-x-0 translate-y-0 gap-0 overflow-hidden rounded-none border-0 p-0",
      "sm:left-[50%] sm:top-[50%] sm:h-auto sm:max-h-[92dvh] sm:w-[96vw] sm:max-w-5xl sm:translate-x-[-50%] sm:translate-y-[-50%] sm:rounded-lg sm:border",
      className,
    )}
    {...props}
  />
...
export function MobileDetailDialogHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <DialogHeader
      className={cn("shrink-0 border-b bg-background px-4 py-4 pr-14 text-left sm:px-6", className)}
      {...props}
    />
  );
}
...
export function MobileDetailDialogBody({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn("min-h-0 min-w-0 flex-1 overflow-y-auto overscroll-contain px-3 py-4 sm:px-6 sm:py-5", className)}
      {...props}
    />
  );
}
...
export function MobileDetailDialogFooter({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "grid shrink-0 grid-cols-2 gap-2 border-t bg-background/95 px-3 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] backdrop-blur",
